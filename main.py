...
class MedicalItem(BaseItem):
    ...
    name: str
    physical_description: str = Field(description="A description of the medical item. Should be at least a sentence long.", min_length=1)
    position_in_location: str = Field(description="The position of the item in the location. Can be a room, a building, a city, a forest, etc. The position of the item in the location.", examples=[
...
class WornClothing(BaseModel):
    """WornClothing Schema - The clothes that a character has on them. This includes all worn clothing, armour and accessories. If the character has no equipment on them, this whole object should be null."""
...
class Character(BaseModel):
    ...
    id: str = Field(default_factory=uuid4, pattern="^([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})$")
    ...
    def get_physical_description(self):
        ...
        if self.worn_clothing.upperbodywear:
            ...
            if not self.worn_clothing.upperbodywear.covers_breasts and not self.worn_clothing.fullbodywear.covers_breasts:
                description += f"{self.naked_body_part_descriptions.naked_bare_chest_descriptioniption}\n"
            if not self.worn_clothing.upperbodywear.covers_belly and not self.worn_clothing.fullbodywear.covers_belly:
        ...
        if self.worn_clothing.lowerbodywear:
            ...
            if not self.worn_clothing.lowerbodywear.covers_legs and not self.worn_clothing.fullbodywear.covers_legs:
                description += f"{self.naked_body_part_descriptions.legs_description}\n"
            if not self.worn_clothing.lowerbodywear.covers_genitals and not self.worn_clothing.fullbodywear.covers_genitals:
                description += f"{self.naked_body_part_descriptions.naked_genital_description}\n"
            if not self.worn_clothing.lowerbodywear.covers_butt and not self.worn_clothing.fullbodywear.covers_butt:
            ...
            if not self.worn_clothing.fullbodywear.covers_genitals:
                description += f"{self.naked_body_part_descriptions.naked_genital_description}\n"
            if not self.worn_clothing.fullbodywear.covers_butt:
                description += f"{self.naked_body_part_descriptions.butt_description}\n"
        ...
        if self.stats.hp <= 0:
            ...
            if self.equiped_weapon:
                description += f"{self.get_pronouns()['subject'].capitalize()} is holding a {self.equiped_weapon.physical_description}."
    ...
    def get_equipment_description(self):
        ...
        if self.worn_clothing.footwear:
            description += f"{self.get_pronouns()['subject'].capitalize()}'s wearing {self.worn_clothing.footwear.physical_description[0].lower()}{self.worn_clothing.footwear.physical_description[1:]}. "
        if self.equiped_weapon:
            description += f"{self.get_pronouns()['subject'].capitalize()} is holding a {self.equiped_weapon.physical_description[0].lower()}{self.equiped_weapon.physical_description[1:]}. "
...
class Location(BaseModel):
    ...
    objects_in_location: list[Union[Item,Food,Weapon,Headwear,Footwear,Gloves,LowerBodywear,UpperBodywear,Accessory,MedicalItem]] = Field(description="A list of objects in the location. Each item should have a name, description, value, weight. All items in the location. This can be furniture, or small objects that characters can interact with. If the item is food, it should have a hunger restored and thirst restored value. If the item is a weapon, it should have a damage value and required SPECIAL stats. If an item is a weapon, it MUST have a damage value and required SPECIAL stats. If an item is food, it MUST have a hunger restored and thirst restored value.")
...
class SomeItem(BaseModel):
    ...
    item: Union[Item,Food,Weapon,Headwear,Footwear,Gloves,LowerBodywear,UpperBodywear,Accessory,MedicalItem]
...
class TextAIventureEngine():
        ...
        while story == None:
            try:
                ...
                for character in story.starting_location.npcs_in_location:
                    ...
                    # if character.worn_clothing.top_underwear != None:
                    #     if character.worn_clothing.top_underwear.name.lower() == "none" or character.worn_clothing.top_underwear.name.lower() == "null" or character.worn_clothing.top_underwear.name.strip() == "":
                    #         character.worn_clothing.top_underwear = None
    ...
    def generate_decisions_for_character(self, character:Character):
        ...
        # messages.append({
        #     "role": "system",
        #     "content": f"{character.full_name} is here.\n{character.get_physical_description()}"
        # })
        messages.append({
            "role": "user",
    ...
    def say(self, character:Character, message:str):
        self.messages_db.add(documents=[message], metadatas=[{
            "id": character.id,
        }], ids=[message["id"]])
        print(f"{character.full_name} says: {message}")
        ...
        for character in self.current_location.npcs_in_location:
            ...
            if character.stats.hp > 0 and character.processing:
                ...
                for decision in decisions.decisions:
                    if decision.type_string == "Chat":
                        print(f"{character.full_name} says: {decision.message}")
                    elif decision.type_string == "Pickup":
                        for item in self.current_location.objects_in_location:
                            if item.name == decision.item:
                                ...
                                break
                    elif decision.type_string == "Drop":
                        for item in character.inventory:
                            if item.name == decision.item:
                                character.inventory.remove(item)
                        ...
                        if character.equiped_weapon != None:
                            if character.equiped_weapon.name == decision.item:
                                ...
                                print(f"{character.full_name} unequips the {decision.item}.")
                                break
                    elif decision.type_string == "Equip":
                        for item in character.inventory:
                            if item.name == decision.item:
...
def print_current_screen():
    ...
    if len(text_adventure.current_location.objects_in_location) > 0:
        ...
        for item in text_adventure.current_location.objects_in_location:
            ...
            description += f"There's a \"{item.name}\" on the ground.\n" # {position_in_location}
    description = description.strip()
...
while True: # Main game loop
    ...
    if action.lower() == "help" or action.lower() == "h" or action.lower() == "?":
        ...
        if at == "":
            ...
            if player.worn_clothing.headwear:
                print(f"{player.full_name} is wearing {player.worn_clothing.headwear.physical_description} on their head.")
            if player.worn_clothing.fullbodywear:
                print(f"{player.full_name} is wearing {player.worn_clothing.fullbodywear.physical_description} on their body.")
            if player.worn_clothing.upperbodywear:
                print(f"{player.full_name} is wearing {player.worn_clothing.upperbodywear.physical_description} on their upper body.")
            if player.worn_clothing.gloves:
                print(f"{player.full_name} is wearing {player.worn_clothing.gloves.physical_description} on their hands.")
            if player.worn_clothing.gloves:
                print(f"{player.full_name} is wearing {player.worn_clothing.gloves.physical_description} on their hands.")
            if player.worn_clothing.lowerbodywear:
                print(f"{player.full_name} is wearing {player.worn_clothing.lowerbodywear.physical_description} on their lower body.")
            if player.worn_clothing.footwear:
                print(f"{player.full_name} is wearing {player.worn_clothing.footwear.physical_description} on their feet.")
            if player.equiped_weapon:
                print(f"{player.full_name} is holding {player.equiped_weapon.physical_description}.")
        else:
            item_found = False
            for item in text_adventure.current_location.objects_in_location:
        ...
        if player.worn_clothing.headwear:
            print(f"Headwear: {player.worn_clothing.headwear.name} - {player.worn_clothing.headwear.physical_description}")
        if player.worn_clothing.fullbodywear:
            print(f"Full Body Wear: {player.worn_clothing.fullbodywear.name} - {player.worn_clothing.fullbodywear.physical_description}")
        if player.worn_clothing.upperbodywear:
            print(f"Upper Body Wear: {player.worn_clothing.upperbodywear.name} - {player.worn_clothing.upperbodywear.physical_description}")
        if player.worn_clothing.gloves:
            print(f"Gloves: {player.worn_clothing.gloves.name} - {player.worn_clothing.gloves.physical_description}")
        if player.worn_clothing.lowerbodywear:
            print(f"Lower Body Wear: {player.worn_clothing.lowerbodywear.name} - {player.worn_clothing.lowerbodywear.physical_description}")
        if player.worn_clothing.footwear:
            print(f"Footwear: {player.worn_clothing.footwear.name} - {player.worn_clothing.footwear.physical_description}")
        if player.equiped_weapon:
            print(f"Equiped Weapon: {player.equiped_weapon.name} - {player.equiped_weapon.physical_description}")
            print(f"Damage: {player.equiped_weapon.damage}")
        if len(player.worn_clothing.accessories) > 0:
            print("Accessories:")
            for item in player.worn_clothing.accessories:
                print(f"{item.name} - {item.physical_description}")
        ...
        if player.worn_clothing.headwear:
            if item_to_inspect.lower() in player.worn_clothing.headwear.name.lower():
                for key, value in player.worn_clothing.headwear.__dict__.items():
                    print(f"{key.capitalize()}: {str(value)}")
                item_found = True
        if player.worn_clothing.fullbodywear:
            if item_to_inspect.lower() in player.worn_clothing.fullbodywear.name.lower():
                for key, value in player.worn_clothing.fullbodywear.__dict__.items():
                    print(f"{key.capitalize()}: {str(value)}")
                item_found = True
        if player.worn_clothing.upperbodywear:
            if item_to_inspect.lower() in player.worn_clothing.upperbodywear.name.lower():
                for key, value in player.worn_clothing.upperbodywear.__dict__.items():
                    print(f"{key.capitalize()}: {str(value)}")
                item_found = True
        if player.worn_clothing.gloves:
            if item_to_inspect.lower() in player.worn_clothing.gloves.name.lower():
                for key, value in player.worn_clothing.gloves.__dict__.items():
                    print(f"{key.capitalize()}: {str(value)}")
                item_found = True
        if player.worn_clothing.lowerbodywear:
            if item_to_inspect.lower() in player.worn_clothing.lowerbodywear.name.lower():
                for key, value in player.worn_clothing.lowerbodywear.__dict__.items():
                    print(f"{key.capitalize()}: {str(value)}")
                item_found = True
        if player.worn_clothing.footwear:
            if item_to_inspect.lower() in player.worn_clothing.footwear.name.lower():
                for key, value in player.worn_clothing.footwear.__dict__.items():
                    print(f"{key.capitalize()}: {str(value)}")
                item_found = True
        for item in player.worn_clothing.accessories:
            if item_to_inspect.lower() in item.name.lower():
                ...
                break
        if player.equiped_weapon:
            if item_to_inspect.lower() in player.equiped_weapon.name.lower():
                for key, value in player.equiped_weapon.__dict__.items():
                    print(f"{key.capitalize()}: {str(value)}")
                item_found = True
        for item in player.inventory:
            if item_to_inspect.lower() in item.name.lower():
            ...
            for character in text_adventure.current_location.npcs_in_location:
                ...
                if character.worn_clothing.headwear != None:
                    if item_to_take.lower() in character.worn_clothing.headwear.name.lower() or item_to_take.lower() in character.worn_clothing.headwear.physical_description.lower():
                        if character.stats.hp <= 0:
                            item_found = True
                            player.inventory.append(character.worn_clothing.headwear)
                ...
                if character.worn_clothing.upperbodywear != None:
                    if item_to_take.lower() in character.worn_clothing.upperbodywear.name.lower() or item_to_take.lower() in character.worn_clothing.upperbodywear.physical_description.lower():
                        if character.stats.hp <= 0:
                            item_found = True
                            player.inventory.append(character.worn_clothing.upperbodywear)
                ...
                if character.worn_clothing.fullbodywear != None:
                    if item_to_take.lower() in character.worn_clothing.fullbodywear.name.lower() or item_to_take.lower() in character.worn_clothing.fullbodywear.physical_description.lower():
                        if character.stats.hp <= 0:
                            item_found = True
                            player.inventory.append(character.worn_clothing.fullbodywear)
                ...
                if character.worn_clothing.gloves != None:
                    if item_to_take.lower() in character.worn_clothing.gloves.name.lower() or item_to_take.lower() in character.worn_clothing.gloves.physical_description.lower():
                        if character.stats.hp <= 0:
                            item_found = True
                            player.inventory.append(character.worn_clothing.gloves)
                ...
                if character.worn_clothing.lowerbodywear != None:
                    if item_to_take.lower() in character.worn_clothing.lowerbodywear.name.lower() or item_to_take.lower() in character.worn_clothing.lowerbodywear.physical_description.lower():
                        if character.stats.hp <= 0:
                            item_found = True
                            player.inventory.append(character.worn_clothing.lowerbodywear)
                ...
                if character.worn_clothing.footwear != None:
                    if item_to_take.lower() in character.worn_clothing.footwear.name.lower() or item_to_take.lower() in character.worn_clothing.footwear.physical_description.lower():
                        if character.stats.hp <= 0:
                            item_found = True
                            player.inventory.append(character.worn_clothing.footwear)
                ...
                if character.equiped_weapon != None:
                    if item_to_take.lower() in character.equiped_weapon.name.lower() or item_to_take.lower() in character.equiped_weapon.physical_description.lower():
                        if character.stats.hp <= 0:
                            item_found = True
                            player.inventory.append(character.equiped_weapon)
        ...
        for item in player.inventory:
            if item_to_equip.lower() in item.name.lower() or item_to_equip.lower() in item.physical_description.lower():
                if item.type_string.lower() == "weapon":
                    ...
                    player.worn_clothing.headwear = item
                    player.inventory.remove(item)
                    for location in text_adventure.locations:
                        if location.name == text_adventure.current_location.name:
                            location.objects_in_location = text_adventure.current_location.objects_in_location
                    ...
                    player.worn_clothing.upperbodywear = item
                    player.inventory.remove(item)
                    for location in text_adventure.locations:
                        if location.name == text_adventure.current_location.name:
                            location.objects_in_location = text_adventure.current_location.objects_in_location
                    ...
                    player.worn_clothing.lowerbodywear = item
                    player.inventory.remove(item)
                    for location in text_adventure.locations:
                        if location.name == text_adventure.current_location.name:
                            location.objects_in_location = text_adventure.current_location.objects_in_location
                    ...
                    player.worn_clothing.footwear = item
                    player.inventory.remove(item)
                    for location in text_adventure.locations:
                        if location.name == text_adventure.current_location.name:
                            location.objects_in_location = text_adventure.current_location.objects_in_location
                    ...
                    player.worn_clothing.gloves = item
                    player.inventory.remove(item)
                    for location in text_adventure.locations:
                        if location.name == text_adventure.current_location.name:
                            location.objects_in_location = text_adventure.current_location.objects_in_location
                    ...
                    player.worn_clothing.accessories.append(item)
                    player.inventory.remove(item)
                    for location in text_adventure.locations:
                        if location.name == text_adventure.current_location.name:
                            location.objects_in_location = text_adventure.current_location.objects_in_location
        ...
        item_found = False
        if player.equiped_weapon != None:
            if item_to_unequip.lower() in player.equiped_weapon.name.lower() or item_to_unequip.lower() in player.equiped_weapon.physical_description.lower():
                player.inventory.append(player.equiped_weapon)
                player.equiped_weapon = None
                item_found = True
                print(f"You unequipped the {item_to_unequip}.")
        if player.worn_clothing.headwear != None:
            if item_to_unequip.lower() in player.worn_clothing.headwear.name.lower() or item_to_unequip.lower() in player.worn_clothing.headwear.physical_description.lower():
                player.inventory.append(player.worn_clothing.headwear)
                player.worn_clothing.headwear = None
                item_found = True
                print(f"You unequipped the {item_to_unequip}.")
        if player.worn_clothing.upperbodywear != None:
            if item_to_unequip.lower() in player.worn_clothing.upperbodywear.name.lower() or item_to_unequip.lower() in player.worn_clothing.upperbodywear.physical_description.lower():
                player.inventory.append(player.worn_clothing.upperbodywear)
                player.worn_clothing.upperbodywear = None
                item_found = True
                print(f"You unequipped the {item_to_unequip}.")
        if player.worn_clothing.fullbodywear != None:
            if item_to_unequip.lower() in player.worn_clothing.fullbodywear.name.lower() or item_to_unequip.lower() in player.worn_clothing.fullbodywear.physical_description.lower():
                player.inventory.append(player.worn_clothing.fullbodywear)
                player.worn_clothing.fullbodywear = None
                item_found = True
                print(f"You unequipped the {item_to_unequip}.")
        if player.worn_clothing.gloves != None:
            if item_to_unequip.lower() in player.worn_clothing.gloves.name.lower() or item_to_unequip.lower() in player.worn_clothing.gloves.physical_description.lower():
                player.inventory.append(player.worn_clothing.gloves)
                player.worn_clothing.gloves = None
                item_found = True
                print(f"You unequipped the {item_to_unequip}.")
        if player.worn_clothing.lowerbodywear != None:
            if item_to_unequip.lower() in player.worn_clothing.lowerbodywear.name.lower() or item_to_unequip.lower() in player.worn_clothing.lowerbodywear.physical_description.lower():
                player.inventory.append(player.worn_clothing.lowerbodywear)
                player.worn_clothing.lowerbodywear = None
                item_found = True
                print(f"You unequipped the {item_to_unequip}.")
        if player.worn_clothing.footwear != None:
            if item_to_unequip.lower() in player.worn_clothing.footwear.name.lower() or item_to_unequip.lower() in player.worn_clothing.footwear.physical_description.lower():
                player.inventory.append(player.worn_clothing.footwear)
                player.worn_clothing.footwear = None
                item_found = True
                print(f"You unequipped the {item_to_unequip}.")
        if len(player.worn_clothing.accessories) > 0:
            for item in player.worn_clothing.accessories:
                if item_to_unequip.lower() in item.name.lower() or item_to_unequip.lower() in item.physical_description.lower():
                    player.inventory.append(item)
                    player.worn_clothing.accessories.remove(item)
                    item_found = True
                    print(f"You unequipped the {item_to_unequip}.")
        if not item_found:
        ...
        for character in text_adventure.current_location.npcs_in_location:
            if character_to_attack.lower() in character.full_name.lower() or character_to_attack.lower() in character.get_unknown_description().lower():
                character_found = True
                character.stats.hp -= player.equiped_weapon.damage
                if character.stats.hp <= 0:
                    print(f"You attacked {character.full_name} with your {player.worn_clothing['equiped_weapon'].name} and killed them.")
                else:
                    print(f"You attacked {character.full_name} with your {player.worn_clothing['equiped_weapon'].name}.")
